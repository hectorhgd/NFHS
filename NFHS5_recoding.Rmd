...
-   Exploring and looking at the distribution of the variables of interest.

-   Obtain a dataset only containing the recoded variables of interest.
...
```{r}
nfhs5 <- readRDS("IABR7AFL_under5_short.Rds")
```
...
tab1(nfhs5$outcome_fever_f)
tab1(nfhs5$outcome_f)
```
## Selecting only children with ARI
Children under 3 years old whose mothers reported ARI symptoms in the 2 weeks before the interview.
```{r}
nfhs5 <- nfhs5 %>% filter(ari == 1 & b5 == 1 & age < 36)
```


## Children with LRTI that were given antibiotics
...
```{r}
nfhs5 <- nfhs5 %>% rename(place = v025)
```
Now let's create a factor version of this variable, it might be useful in the future.
...
```{r}
tab1(nfhs5$place, sort.group = "decreasing", cum.percent = TRUE)
```


Alright, that was a very nice first attempt! Now let's re-check with the other variable that contains the same info, v102.

```{r}
table(nfhs5$v102, exclude = NULL)
```

So the results are the same, we can move on to the next variable.
...
```{r}
tab1(nfhs5$v130, sort.group = "decreasing", cum.percent = TRUE)
```
...
```{r}
nfhs5$religion_f <- factor(nfhs5$religion, labels = c("Hindu", "Muslim", "Others"))
```

Checking that everything is alright:

```{r}
tab1(nfhs5$religion_f)
tab1(nfhs5$religion)
...
nfhs5 <- nfhs5 %>% 
  mutate(state_new = case_when(
    state == 35 ~ 1, # Andaman And Nicobar Islands 
    state == 28 ~ 2, # Andhra Pradesh
    state == 12 ~ 3, # Arunachal Pradesh
    state == 18 ~ 4, # Assam
    state == 10 ~ 5, # Bihar
    state == 4 ~  6, # Chandigarh
    state == 22 ~ 7, # Chhattisgarh
    state == 25 ~ 8, # Dadra & Nagar Haveli And Daman & Diu
    state == 30 ~ 10, # Goa
    state == 24 ~ 11, # Gujarat
    state == 6 ~ 12 , # Haryana
    state == 2 ~ 13 , # Himachal Pradesh
    state == 1 ~ 14,  # Jammu And Kashmir
    state == 37 ~ 14, # Ladakh
    state == 20 ~ 15, # Jharkhand
    state == 29 ~ 16, # Karnataka
    state == 32 ~ 17, # Kerala
    state == 31 ~ 18, # Lakshadweep
    state == 23 ~ 19, # Madhya Pradesh
    state == 27 ~ 20, # Maharashtra
    state == 14 ~ 21, # Manipur
    state == 17 ~ 22, # Meghalaya
    state == 15 ~ 23, # Mizoram
    state == 13 ~ 24, # Nagaland
    state == 7  ~ 25, # Delhi
    state == 21 ~ 26, # Odisha
    state == 34 ~ 27, # Puducherry
    state == 3 ~ 28,  # Punjab
    state == 8 ~ 29,  # Rajasthan
    state == 11 ~ 30, # Sikkim
    state == 33 ~ 31, # Tamil Nadu
    state == 16 ~ 32, # Tripura
    state == 9 ~ 33,  # Uttar Pradesh
    state == 5 ~ 34,  # Uttarakhand
    state == 19 ~ 35, # West Bengal
    state == 36 ~ 36  # Telangana
      )) %>% 
  mutate(state_f = factor(state_new, c(1:8,10:36),labels =  c("Andaman And Nicobar Islands","Andhra Pradesh","Arunachal Pradesh", "Assam","Bihar","Chandigarh","Chhattisgarh", "Dadra & Nagar Haveli And Daman & Diu", "Goa","Gujarat","Haryana","Himachal Pradesh","Jammu And Kashmir", "Jharkhand","Karnataka","Kerala","Lakshadweep","Madhya Pradesh", "Maharashtra","Manipur","Meghalaya","Mizoram","Nagaland","Delhi","Odisha","Puducherry","Punjab","Rajasthan","Sikkim","Tamil Nadu","Tripura","Uttar Pradesh","Uttarakhand","West Bengal","Telangana")))
tab1(nfhs5$state)        
tab1(nfhs5$state_new)                              
tab1(nfhs5$state_f)
...
Coded in `sdist`. Rename as `district` and create factor variable `district_f`:
...
```
```{r}
tab1(nfhs5$district_f)
```

### Wealth index

The are different wealth index depending if they are grouped by state, rural/urban. Let's use the global one `v190`
...
Coded in v136. Rename as `hhsize`, dbl variable. Kept as continuous.
...
```
```{r}
nfhs5 <- mutate(nfhs5, hhsize_f = case_when(hhsize >= 10  ~ ">=10",
...
```{r}
tab1(nfhs5$media.radio_f)
tab1(nfhs5$media.reading_f)
tab1(nfhs5$media.tv_f)
```
...
Checking-up:
```{r}
#identical(nfhs5$media.reading, IAKR73FL_DC$v157)
#identical(nfhs5$media.radio, IAKR73FL_DC$v158)
#identical(nfhs5$media.tv, IAKR73FL_DC$v159)
```
...
```{r}
nfhs5 <- nfhs5 %>% rename(water.source = v113)
nfhs5 <- nfhs5 %>% 
...
nfhs5 <- nfhs5 %>% 
  mutate(water.source_g = case_when(
    ...
    water.source < 32 | water.source == 41 | water.source == 51 | water.source == 92 ~ 1,
    water.source == 61 | water.source == 62| water.source == 71 ~ 1,
    water.source >= 32 ~ 2))

tab1(nfhs5$water.source_g)
...
```{r}
tab1(nfhs5$v007)
hhsurvey5_small <- readRDS("hhsurvey5_small_merge.Rds")
hhsurvey5_small <- hhsurvey5_small %>% 
  rename(v001 = hv001) %>%
  rename(v002 = hv002) 

nfhs5 <- nfhs5 %>% 
  left_join(hhsurvey5_small, by = c("v001", "v002"))
...
tab1(nfhs5$handwash.soap_f)
tab1(nfhs5$handwash.ash_f)
tab1(nfhs5$handwash.any_f)
```
...
```{r}
# nfhs5 <- nfhs5 %>% rename(age = hw1) Coded already from CDC in NFHS5_clean

tab1(nfhs5$age)

nfhs5 <- nfhs5 %>%  mutate(age_g = case_when(
...
```{r}
nfhs5 <- nfhs5 %>% rename(delivery = m15)

nfhs5 <- nfhs5 %>% mutate(delivery_f = to_factor(delivery))

tab1(nfhs5$delivery_f)
```
Recoding answers for regrouping:
```{r}
nfhs5 <- nfhs5 %>% mutate(delivery_g = case_when(
...
tab1(nfhs5$delivery_g)
```
Factoring new regrouped variable:

```{r}
nfhs5 <- nfhs5 %>% mutate(delivery_g_f = factor(delivery_g, c(1,2,3,4), c("Public", "Private", "Home", "Other/NGO")))
tab1(nfhs5$delivery_g_f)
```
...
```{r}
nfhs5 <- nfhs5 %>% rename(csection = m17)
nfhs5 <- nfhs5 %>% mutate(csection_f = to_factor(csection))

tab1(nfhs5$csection_f)
```
### Birth order

```{r}
lookfor(nfhs5, "birth order")
tab1(nfhs5$bord)
```

```{r}
nfhs5 <- nfhs5 %>% mutate(bord_g = case_when(
  bord > 3 ~ ">=4",
...
```{r}
nfhs5 <- nfhs5 %>% mutate(bord_g_f = factor(bord_g, c("1", "2","3", ">=4"), c("1", "2","3", ">=4")))
tab1(nfhs5$bord_g_f)
```
...
tab1(nfhs5$interview.month)
tab1(nfhs5$interview.quarter_f)
...
```{r}
nfhs5 <- nfhs5 %>% rename(stunting = hw70)
nfhs5 <- nfhs5 %>% rename(wasting = hw72)
nfhs5 <- nfhs5 %>% rename(underweight = hw71)
nfhs5 <- nfhs5 %>% rename(bmi = hw73)
```
#### Stunting
Regrouping:
...
nfhs5 <- nfhs5 %>% mutate(stunting_g_f = factor(stunting_g, c(0, 1, 2), c("Not stunted", "Moderately stunted", "Severely stunted")))
tab1(nfhs5$stunting_g)
tab1(nfhs5$stunting_g_f)
```
#### Wasting
```{r}
# nfhs5 <- nfhs5 %>% In case we want to regroup it with overweight category
...
nfhs5 <- nfhs5 %>% mutate(wasting_g_f = factor(wasting_g, c(0, 1, 2), c("Not wasted", "Moderately wasted", "Severely wasted")))
tab1(nfhs5$wasting_g)
tab1(nfhs5$wasting_g_f)
```
#### Underweight
...
nfhs5 <- nfhs5 %>% mutate(underweight_g_f = factor(underweight_g, c(0, 1), c("No", "Yes")))
tab1(nfhs5$underweight_g)
tab1(nfhs5$underweight_g_f)
...
```{r}
nfhs5 <- nfhs5 %>% rename(breastfed = m4)
...
### Both Mothers at home

v504 -> together

```{r}
nfhs5 <- nfhs5 %>% rename(together = v504) %>% 
  mutate(together_f = to_factor(together))
tab1(nfhs5$together_f)
```
### Mother's marital status

```{r}
look_for(nfhs5,"marital")
look_for(nfhs5, "s301")
...
tab1(nfhs5$m.tobacco)
tab1(nfhs5$m.tobacco_f)
```
...
tab1(nfhs5$pregnancy)
tab1(nfhs5$pregnancy_f)
```
...
h46b     days after fever or cough sought advice or treatment
...
tab1(nfhs5$days.after)
tab1(nfhs5$days.after_g_f)
```
...
nfhs5 <- nfhs5 %>% 
  ...
  mutate(drink_f = factor(drink, c(0,2,3,4,5), c("Nothing to drink", "Much less", "Somewhat less", "About the same", "More")))%>% 
...
Combined Stopped food and Never gave food
...
#### Visit in the last three months: three variables

 888 s359     met with an anm or lhv in last 3 months                           dbl+lbl 
893  s361     met with an anganwadi worker, asha or other community health worker in last 3 mo dbl+lbl  [0] no 
                                                                                                         [1] yes
923 s368     visited a health facility or camp for self or children in last 3~ dbl+lbl 

```{r}
nfhs5 <- nfhs5 %>% 
  rename(visit3mo = s361) %>% 
  mutate(visit3mo_f = to_factor(visit3mo))

tab1(nfhs5$visit3mo_f)
```
...
tab1(nfhs5$distance_f)
tab1(nfhs5$distance_g_f)
```
...
tab1(nfhs5$vaccination.ever_f)
```

Create variables for 3+ doses of DPT, 3+ doses of polio, bcg, and measles vaccination, as well as one for having all ("fully immunized"):
...
s558     child received benefits from anganwadi/icds centre, last 12 months               dbl+lbl  [0] no                                   
                                                                                                   [1] yes
...
tab1(nfhs5$parasites)
tab1(nfhs5$parasites_f)
```
...
```{r}
nfhs5 <- nfhs5 %>% 
  rename(allowed = s930b) %>% 
  mutate(allowed_f = to_factor(allowed))

tab1(nfhs5$allowed_f)
```
...
```{r}
saveRDS(nfhs5_ari, "nfhs5_ari.Rds")
saveRDS(nfhs5_ari, "C:/Hector/IDOH/McGill/Master thesis/NFHS/Datasets/NFHS_analysis/nfhs5_ari.Rds")
```
